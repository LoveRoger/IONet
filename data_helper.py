...
def get_data_info(folder_list, seq_len_range, overlap, sample_times=1, pad_y=False, shuffle=False, sort=True):
    ...
    data = {'seq_len': X_len, 'imu_data': X_data, 'pose': Y}
    df = pd.DataFrame(data, columns = ['seq_len', 'imu_data', 'pose'])
...
def get_partition_data_info(partition, folder_list, seq_len_range, overlap, sample_times=1, pad_y=False, shuffle=False, sort=True):
    ...
    for part in range(2):
        for folder in folder_list:
            start_t = time.time()
            poses = np.load('{}{}.npy'.format(par.pose_dir_npy, folder))  # (n_images, 6)
            imu_data = np.load('{}{}.npy'.format(par.pose_dir_npy, folder))  # (n_images, 6)
            ###imu_data.sort() # does it have a meaning???


            # Get the middle section as validation set
            n_val = int((1-partition)*len(imu_data))
            st_val = int((len(imu_data)-n_val)/2)
            ed_val = st_val + n_val
            print('st_val: {}, ed_val:{}'.format(st_val, ed_val))
            if part == 1:
                imu_data = imu_data[st_val:ed_val]
                poses = poses[st_val:ed_val]
            else:
                imu_data = imu_data[:st_val] + imu_data[ed_val:]
        ...
        # Convert to pandas dataframes
        data = {'seq_len': X_len[part], 'imu_data': X_data[part], 'pose': Y[part]}
        df = pd.DataFrame(data, columns = ['seq_len', 'imu_data', 'pose'])
        # Shuffle through all videos
        if shuffle:
            df = df.sample(frac=1)
...
class ImuSeqDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        print("shape of raw gt: ")
        print(np.shape(raw_groundtruth))
        ...
        imu_data_sequence = self.imu_data_arr[index]
        sequence_len = torch.tensor(self.seq_len_list[index])  #sequence_len = torch.tensor(len(imu_data_sequence))
        
        imu_sequence = torch.FloatTensor(imu_data_sequence)
        return (sequence_len, imu_sequence, groundtruth_sequence)
