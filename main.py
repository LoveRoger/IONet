...
from model import IONet
from data_helper import get_data_info, SortedRandomBatchSampler, ImuSeqDataset, get_partition_data_info
...
if os.path.isfile(par.train_data_info_path) and os.path.isfile(par.valid_data_info_path):
	...
	if par.partition != None:
		...
		train_df, valid_df = get_partition_data_info(partition, par.train_imus, par.seq_len, overlap=1, sample_times=par.sample_times, shuffle=True, sort=True)
	else:
		train_df = get_data_info(folder_list=par.train_imus, seq_len_range=par.seq_len, overlap=1, sample_times=par.sample_times)
		valid_df = get_data_info(folder_list=par.valid_imus, seq_len_range=par.seq_len, overlap=1, sample_times=par.sample_times)
	# save the data info
...
train_dataset = ImuSeqDataset(train_df)
...
valid_dataset = ImuSeqDataset(valid_df)
...
# Model
M_ionet = IONet(par.batch_norm)
use_cuda = torch.cuda.is_available()
if use_cuda:
    print('CUDA used.')
    M_ionet = M_ionet.cuda()
...
if par.pretrained_flownet and not par.resume:
	...
	model_dict = M_ionet.state_dict()
	update_dict = {k: v for k, v in pretrained_w['state_dict'].items() if k in model_dict}
	model_dict.update(update_dict)
	M_ionet.load_state_dict(model_dict)


# Create optimizer
if par.optim['opt'] == 'Adam':
	optimizer = torch.optim.Adam(M_ionet.parameters(), lr=0.001, betas=(0.9, 0.999))
elif par.optim['opt'] == 'Adagrad':
	optimizer = torch.optim.Adagrad(M_ionet.parameters(), lr=par.optim['lr'])
elif par.optim['opt'] == 'Cosine':
	optimizer = torch.optim.SGD(M_ionet.parameters(), lr=par.optim['lr'])
...
if par.resume:
	M_ionet.load_state_dict(torch.load(par.load_model_path))
...
for ep in range(par.epochs):
	...
	for _, t_x, t_y in train_dl:
		...
		ls = M_ionet.step(t_x, t_y, optimizer).data.cpu().numpy()
	...
	for _, v_x, v_y in valid_dl:
		...
		v_ls = M_ionet.get_loss(v_x, v_y).data.cpu().numpy()
	...
	if loss_mean_valid < min_loss_v and ep % check_interval == 0:
		...
		torch.save(M_ionet.state_dict(), par.save_model_path+'.valid')
	...
	if loss_mean < min_loss_t and ep % check_interval == 0:
		...
		torch.save(M_ionet.state_dict(), par.save_model_path+'.train')
